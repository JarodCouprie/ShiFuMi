...
//Pierre Papier et Ciseaux sont les diffférents éléments qui permettent de jouer au jeu
...
var joueur = null;//Choix du joueur entre les éléments
var ordinateur = null;//Choix du joueur entre les éléments
var score = 0;//Score du joueur
var scoreOrdi = 0;//Score de l'ordinateur
var choixJeu = ["pierre","papier","ciseaux"];//Tableau permettant aux fonctions de voir ce qui a été choisi l'ordinateur à partir d'un nombre aléatoire donnant l'index du tableau
...
function choixRandom(){
    return Math.floor(Math.random()*3);//On multiplie la valeur par 3 car il y a 3 valeurs dans notre tableau [choixJeu] et que l'on veut récupérer leur index [i]
}

//Création des évênements pour répérer lorsque le joueur clique sur un des éléments (Pierre, Papier ou Ciseaux)
//Les trois boutons créés vont suivre les mêmes règles et le clique sur chacun d'eux aura les mêmes conséquences
...
pierre.addEventListener("click" , function(){
    ...
    ordinateur = choixJeu[choixRandom()];/*La variable ordinateur prend une variable définie par un nombre aléatoire donné par la fonction précente 
...
papier.addEventListener("click" , function(){
    joueur = "papier";
    ordinateur = choixJeu[choixRandom()];
...
function confrontation(joueurChoix, ordinateurChoix){//On prendra comme entrée le choix du joueur et la décision pour l'ordinateur
    if (joueurChoix == ordinateurChoix){//Gestion de l'égalité
        ...
        joueurChoix == "ciseaux" && ordinateurChoix == "papier"){//Gestion de la victoire du joueur
